...
* Vendor subscription lifecycle is handled through `POST /api/v1/vendor/subscriptions` (create, idempotent), `POST /api/v1/vendor/subscriptions/cancel` (idempotent), `POST /api/v1/vendor/subscriptions/pause`, `POST /api/v1/vendor/subscriptions/resume`, and `GET /api/v1/vendor/subscriptions` (fetch the single active subscription or `null`). The POSTs require an `Idempotency-Key` and Square customer/payment method IDs; the API mirrors Square state into the local `subscriptions` table and flips `stores.subscription_active`.
