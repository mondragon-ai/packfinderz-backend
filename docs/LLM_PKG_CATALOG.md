...
- `internal/orders` (PF-077) will define `CheckoutGroup`, `VendorOrder`, `OrderLineItem`, and `PaymentIntent` repository layers so the checkout execution path can safely persist the cart snapshot into durable orders; details remain pending until migrations/models are implemented.
