...
## internal/orders
- `internal/orders.Repository` persists `checkout_groups`, `vendor_orders`, `order_line_items`, and `payment_intents` (internal/orders/interfaces.go:1-29) and surfaces `ListBuyerOrders` which joins `payment_intents` + `stores`, filters on `order_status`, `fulfillment_status`, `shipping_status`, `payment_status`, `created_at` range, and a `q` search over buyer/vendor names (internal/orders/repo.go:109-211).
...
- `BuyerOrderFilters` (internal/orders/dto.go:10-19) mirrors repository filters and exposes `Query` for case-insensitive buyer/vendor name matching, while `pkg/db/models/vendor_order.go:12-37` now records `fulfillment_status`, `shipping_status`, and the sequential `order_number` populated via `pkg/migrate/migrations/20260126000001_add_vendor_order_fields.sql:4-51`.
