...
- `Service.List` enforces a non-nil `StoreID`, accepts `Limit`/`Cursor`/`UnreadOnly`, decodes/encodes the cursor via `pagination.ParseCursor`/`EncodeCursor`, and exposes the `List`, `MarkRead`, and `MarkAllRead` operations that API controllers can reuse while validating store context and propagating `pkg/pagination` limits (internal/notifications/service.go:1-87; pkg/pagination/pagination.go:12-40).
