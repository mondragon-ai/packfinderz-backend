...
* [ ] Update order status correctly -> once all pending items are handled move fulfillment (partial|fulfilled) and order status to `ready_for_dispatch`
* [ ] Emit `order_ready_for_dispatch` outbox once all pending items are handled 
...
* [ ] Order status becomes `hold` (hold_for_pickup)
...
## **[PF-092] Buyer Order Cancel + Nudge + Retry Services**
...
* [ ] Inventory released on cancel -> initiate refund if charged (ACH future implimenation stubbed)
* [ ] Nudge emits notification event
* [ ] Retry creates new checkout attempt for that single order only for the particular vendor
...
* Retry must not re-run entire checkout group ONLY the vendors' order that expired.
...
## **[PF-093] Reservation TTL Scheduler (Expire + Nudge)**
