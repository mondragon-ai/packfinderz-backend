...
# Title [PF-XXX]: Implement cmd/analytics-worker Pub/Sub consumer with Redis idempotency gate
...
# Title [PF-XXX]: Implement analytics-worker routing switch and event handler interface contracts
...
# Title [PF-XXX]: Implement BigQuery write client with retries and JSON serialization helpers
...
* What is included
  ...
  * A BigQuery writer package (e.g., `pkg/bigquerywriter` or within existing `pkg/bigquery`) that supports:
...
* [ ] Writer can insert a valid `ad_event_facts` row with `type=charge` and `cost_cents`.
...
# Title [PF-XXX]: Implement marketplace_events row mapping for order_created payload
...
Map the canonical `order_created` payload (order snapshot model) into a flattened `marketplace_events` BigQuery row with embedded `items` JSON, buyer geo fields, and monetary fields. This powers marketplace dashboard queries without Postgres lookups.
...
# Title [PF-XXX]: Implement attribution token decoding and ad conversion record generation
...
Implement the token receipt attribution model for analytics: decode `attributed_ad_tokens[]` from order snapshot payloads and generate `ad_event_facts` conversion rows with attributed revenue based on token target type (store vs product). This enables ROAS computation in BigQuery.
...
# Title [PF-XXX]: Implement marketplace_events handling for order_paid and cash_collected events
...
* [ ] Processing an `order_paid` message results in exactly one `marketplace_events` row inserted with `event_type=order_paid` and revenue cents populated from payload.
* [ ] Processing a `cash_collected` message results in exactly one `marketplace_events` row inserted with `event_type=cash_collected` and revenue cents populated from payload.
...
# Title [PF-XXX]: Implement marketplace_events handling for order_canceled and order_expired events
...
* [ ] Processing `order_canceled` inserts exactly one marketplace row with `event_type=order_canceled` and includes cancellation metadata (reason) in payload if available.
* [ ] Processing `order_expired` inserts exactly one marketplace row with `event_type=order_expired` and includes expiry metadata (ttl_days) in payload if available.
...
# Title [PF-XXX]: Emit ad_daily_charge_recorded analytics events from nightly billing job and write charge rows to ad_event_facts
...
# Title [PF-XXX]: Implement Marketplace Analytics query service using BigQuery SQL
...
# Title [PF-XXX]: Implement Ad Analytics query service using BigQuery SQL
...
# Title [PF-XXX]: Add marketplace analytics API endpoints backed by BigQuery query service
...
# Title [PF-XXX]: Add ad analytics API endpoints backed by BigQuery query service
...
# Title [PF-XXX]: Build analytics ingestion and query test harness plus one-time BigQuery backfill script
