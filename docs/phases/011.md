...
* [ ] Single endpoint sets `read_at` on first call and is a no-op on subsequent calls
* [ ] Bulk endpoint sets `read_at` on all unread notifications for the active store
* [ ] Repeated calls to either endpoint return success without changing state
* [ ] Notifications from other stores cannot be modified
