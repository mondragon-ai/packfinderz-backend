...
## Title [PF-302]: Mirror License Status to Store KYC Status
...
## Title [PF-303]: Emit License Status Outbox Events + Compliance Notifications
...
Emit outbox events whenever a license is approved or rejected, enabling downstream consumers (notifications, analytics) to react asynchronously. Compliance notifications must be sent to both the store and admins.
...
* Consume event to generate notifications:
  ...
  * Admin notified when license enters review queue
...
* Pub/Sub topic: compliance events (new or existing)
...
## Title [PF-304]: Implement License Expiry Scheduler (14-Day Warning + Expiration)
...
* Find licenses expiring in 14 days → notify store
* Find licenses expired today → set status to `expired`
* Mirror store KYC status after expiry
...
* [ ] Store KYC is recalculated after expiry
...
* Worker: `cmd/worker` (scheduler loop)
