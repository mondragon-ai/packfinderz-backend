...
# Title [PF-120]: Implement shared HTTP client helper for integration scripts
...
* will be used inside `/scripts/integration/run.sh`
...
# Title [PF-121]: Implement colored JSON console logger for integration output
...
# Title [PF-122]: Scripted register flow for buyer and vendor stores
...
* password is the `STORE_PASSWORD` env var from the .env file
* Implementation: `scripts/integration/register.sh`, invoked via `make test INTEGRATION_ARGS="--route register"`
* Output: JSON payload with per-store `store_id`, `user_id`, `access_token`, and `refresh_token`
