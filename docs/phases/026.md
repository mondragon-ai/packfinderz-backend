...
# Title [PF-235]: Set payment intent amount and payment method from checkout confirmation
...
# Title [PF-236]: Add repository helper to fetch full checkout result by checkout_group_id
...
Add a repository helper to fetch the complete checkout result by `checkout_group_id` || `cartID`, including vendor orders, payment intents, and relevant metadata. This supports post-checkout flows and tests.

## Scope

* Implement repo helper query by `checkout_group_id` || `cartID`
* Join vendor_orders, payment_intents, order assignments, and related entities
* Return a structured aggregate result
* create `GET /checkout/{'checkout_group_id' || 'cartID'}/confirmation` used post checkout to show final status post inventory reservartion and other statuses. May be we can allow buyer to use as gathering info post checkout to get status update per vendor over time. 
...
# Title [PF-237]: Emit Notifications outbox event for checkout conversion
...
# Title [PF-238]: Emit Analytics outbox event for checkout conversion with attribution
...
* Define/extend analytics outbox payload schema (will be used for both ad and marketplace analytics if `ad_tokens` are present cart/item level)
* Include cart totals, line items, and attribution `ad_tokens`
...
* [ ] Transaction DB failures do not emit. 
...
# Title [INCOMPLETE ❌] [PF-XXX]: Prevent duplicate vendor orders on checkout retry
...
# Title [INCOMPLETE ❌] [PF-XXX]: Prevent duplicate outbox rows on checkout conversion retry
...
# Title [INCOMPLETE ❌] [PF-XXX]: Define outbox payload versioning rules for checkout events
...
# Title [INCOMPLETE ❌] [PF-XXX]: Add checkout regression tests for idempotency and outbox emission
...
* Blocked by: PF-XXX, PF-XXX, PF-XXX, PF-XXX, PF-XXX
* Blocks: None
