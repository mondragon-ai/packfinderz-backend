...
* [ ] Non-agent users cannot access `/api/v1/agent/*` routes
...
## Title [PF-094]: Add agent API route group and base controllers
...
## Title [PF-095]: Create order_assignments migration
...
Add the `order_assignments` table (if it doesnt exist) to persist agent-to-order assignments, including reassignment history and active assignment enforcement.
...
## Title [PF-096]: Auto-assign agent on order ready for dispatch
...
When a vendor order transitions to `hold_for_pickup` (ready for pickup), automatically assign a random available agent and create an active `order_assignments` row.

## Scope

* Detect transition to `hold_for_pickup`
...
* [ ] Assignment is created when order enters `hold_for_pickup`
...
## Title [PF-097]: List unassigned orders for agent dispatch queue
...
Expose an endpoint for agents to view orders in `hold_for_pickup` status that are not yet assigned to any agent.

## Scope

* `GET /api/v1/agent/orders/queue`
* Return orders with `status=hold_for_pickup` and no active assignment
* Paginated response
...
## Title [PF-098]: List active assignments for agent + order detail
...
* `GET /api/v1/agent/orders?...` -> assigned order list
* `GET /api/v1/agent/orders/{orderID}` -> assigned order detail page (line items, order && vendor info, gen order info)
...
## Title [PF-099]: Agent pickup order (in_transit transition)
...
* [ ] Order transitions from `hold`/`hold_for_pickp` → `in_transit`
* [ ] Idempotent requests are supported
* [ ] Invalid state transitions return 422
* [ ] Add goose migration to the `OrderAssignment` model to allow for `pickup_time`, `delivery_time` & cash `pickup_time` fields. Maybe we have two GCS key fields that are nullable as we will soon mandate a signature and create a vector/image to be stored and associated with the assignment for deliver, pickup & cash picked up
...
* Update `vendor_orders.status` `vendor_orders.shipping_status`
...
## Title [PF-100]: Agent deliver order (delivered transition)
...
* [ ] Order transitions to `delivered` (Update `vendor_orders.status` `vendor_orders.shipping_status`)
...
## Title [PF-101]: Record cash collection and settle payment
...
* Refund handling
* Analytics and notificaiton consumers. 
