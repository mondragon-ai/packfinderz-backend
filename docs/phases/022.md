## Title [PF-200]: Add field to agent registration DTO
...
## Title [PF-201]: Add auth middleware tests for token and session edge cases
...
## Title [PF-202]: Add RBAC guard tests for admin and agent routes
...
## Title [PF-203]: Remove store IDs from User model and helpers
...
Remove deprecated store ID fields from the User model to enforce the canonical user–store relationship via membership tables. This reduces ambiguity and aligns with multi-store ownership.

## Scope

* Goose migration to drop store ID columns from users
* Update User model to remove store ID fields
...
* [ ] Database migration removes store ID fields
...
## Title [PF-204]: Remove tokens from auth response bodies and return via headers only
...
## Title [PF-205]: Allow register flow to create a store for an existing user
