...
# Title [PF-178]: Perform inventory reservation at checkout and map failures to line items and vendors
...
* [ ] Vendors with all items rejected are marked rejected (but still returned in the `CartQuote`-like response)
...
# Title [PF-179]: Enforce cart-truth pricing and prevent recomputation during checkout execution
...
# Title [PF-180]: Remove legacy checkout artifacts and migrate fully to cart-truth checkout flow
...
Clean up legacy checkout code paths and artifacts that conflict with the new cart-as-canonical-truth architecture. This includes removing all references to the deprecated `checkout_groups` table and eliminating `AttributedAdClickID` usage end-to-end in favor of `cart_records.ad_tokens`.

The goal is to ensure there is **one authoritative model set** for checkout and orders, aligned with the canonical tables and models in `pkg/db/models`.

## Scope

### What is included

* Remove all usage of `CreateCheckoutGroup` and related repository/service methods
* Delete or refactor code paths that reference the dropped `checkout_groups` table
* Remove `AttributedAdClickID` from:

  * Checkout controller DTOs
  * Checkout service inputs
  * Event payloads and outbox emits
* Ensure checkout and order flows rely exclusively on canonical tables/models:

  * **Cart truth:** `cart_records`, `cart_vendor_groups`, `cart_items`
  * **Order truth:** `vendor_orders`, `order_assignments`, `order_line_items`, `payment_intents`, `payment_methods`
* Update imports, helpers, and services to reference canonical models in `pkg/db/models/*`
* Remove dead code, unused structs, and obsolete helpers tied to legacy checkout flow

### What is explicitly NOT included

* Schema/migration changes (handled in separate migration tickets)
* Refactoring checkout business logic (covered by checkout service tickets)
* Attribution materialization logic (handled in attribution phase)
* Test rewrites beyond minimal fixes required to compile

## Acceptance Criteria

* [ ] No references to `checkout_groups` exist in codebase (services, repos, controllers)
* [ ] No references to `AttributedAdClickID` exist in DTOs, services, or event payloads
* [ ] Checkout and order code compile using only canonical cart and order models
* [ ] All checkout-related reads/writes reference the declared tables of truth
* [ ] Dead code paths related to legacy checkout are fully removed

## Dependencies

### Blocked by

* Checkout service refactor tickets (previous GH pushes)
* Migration that drops `checkout_groups` table

### Blocks

* Stabilization and regression testing phase
* Attribution and analytics correctness verification

## Technical Notes

* Relevant services, tables, APIs, queues, or workers:

  * Services: `checkout.Service`, cart service, orders service
  * Models: `pkg/db/models/cart_*`, `vendor_orders`, `order_line_items`, `payment_intents`
  * Repos: checkout/order repositories referencing legacy group concepts
* Assumptions (explicitly marked):

  * **ASSUMPTION:** All required checkout grouping semantics are now represented by `cart_records.checkout_group_id`.
  * **ASSUMPTION:** `ad_tokens` on `cart_records` fully replace prior ad attribution inputs.

## Out of Scope

* Reworking analytics consumers
