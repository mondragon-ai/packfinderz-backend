...
# Title [PF-XXX]: Build vendor order creation pipeline from cart vendor groups and items
...
* [ ] No pricing or discount recomputation occurs
...
## Title [PF-314]: Enforce vendor eligibility rules and create order line items from cart snapshots
...
## Title [PF-315]: Create payment intents per vendor order using checkout-selected payment method
...
## Title [PF-316]: Perform inventory reservation at checkout and map failures to line items and vendors
...
## Title [PF-317]: Enforce cart-truth pricing and prevent recomputation during checkout execution
...
Ensure checkout execution never recomputes pricing or discounts. Pricing, discounts, and totals must come exclusively from the cart snapshot. The only permitted invalidation at checkout time is inventory reservation failure, which may adjust effective totals by removing rejected items.
...
* Ensure vendor totals are adjusted only due to rejected line items
