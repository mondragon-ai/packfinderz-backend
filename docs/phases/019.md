...
* Relevant packages / files (current examples from thread):
  ...
  * Existing service needing refactor:
    ...
    * `internal/cart/service.go` (contains `QuoteCart`, `GetActiveCart`)
  * Existing controller DTO to stop using:

    * `cartdto.QuoteCartRequest` + `toQuoteCartInput()` mapping into `cartsvc.QuoteCartInput`
...
* **Included**
  ...
  * Persist to `cart_records` fields:
    ...
    * `valid_until` (server-defined window; e.g. now + 5m) -> just means stale but not logic for now.
    * `currency` (default USD if empty) (create enum for USD | BTC | ETH)
...
# Title [PF-161]: Enforce quote-only invariant: no inventory mutation/reservation and no 409 conflicts on shortage (soft quote warnings)
...
# Title [PF-162]: Validate vendor promo ownership and apply promo effects to vendor group totals
...
Add support for **vendor-scoped promo codes** in the cart quote flow by validating promo ownership and conditionally applying promo effects to vendor group totals.

This work ensures promo codes are **recognized, validated, and reflected in vendor group aggregates**, without enforcing redemption limits or checkout-side guarantees.

Promo handling at this stage is **quote-only and informational**, preparing data for later checkout enforcement.

## Scope

### Included

* Accept vendor promo codes as part of the quote input (already defined in `QuoteCartRequest`)
* For each vendor group:

  * Validate that the promo code belongs to the vendor_store_id
  * Validate promo basic eligibility (exists, active, not expired)
* Apply promo effect to vendor group totals **only if valid**

  * Update `cart_vendor_groups.promo` JSON field
  * Adjust `subtotal_cents → total_cents` for that vendor group
* Ensure promo effects apply **only to the owning vendor group**, not the entire cart

### Explicitly NOT Included

* Enforcing usage limits, redemption counts, or exclusivity rules
* Persisting promo redemption events
* Checkout-side enforcement or rollback logic
* Cart-level (cross-vendor) discount logic

## Acceptance Criteria

* [ ] A valid vendor promo code reduces only the corresponding vendor group’s `total_cents` and is persisted in `cart_vendor_groups.promo`
* [ ] Promo codes supplied for a vendor do not affect other vendor groups in the same cart

## Dependencies

* Blocked by:

  * Phase 4: Vendor group aggregation and persistence (`cart_vendor_groups`)
* Blocks:

  * Phase 6: API exposure of promo results
  * Future checkout promo enforcement

## Technical Notes

* Primary integration point: vendor group aggregation step (e.g. `aggregateVendorGroups`)
* Expected data shape:

  * `cart_vendor_groups.promo` JSON:
...
* Promo lookup source:
  ...
  * Vendor-scoped promo table or service (existing or stubbed)
* **ASSUMPTION:** Promo records already exist and are associated with `vendor_store_id`
* **ASSUMPTION:** Promo discount type is fixed-amount at quote time (percentage handling can be added later if already supported)

## Out of Scope

* Promo stacking rules
* Cart-level or multi-vendor promotions
* Analytics or attribution related to promos

---

# Title [PF-163]: Emit vendor-level warnings for invalid or inapplicable promo codes
...
Ensure invalid or inapplicable vendor promo codes are surfaced to the client via **vendor-level warnings**, without failing the cart quote.

This allows the UI to clearly explain why a promo was not applied while still returning an authoritative cart quote.

## Scope

### Included

* Detect promo validation failures per vendor group:

  * Promo does not exist
  * Promo does not belong to vendor
  * Promo inactive or expired
* Emit vendor group warnings using `vendor_group_warning_type = invalid_promo`
* Persist warnings to `cart_vendor_groups.warnings` JSONB
* Ensure quote continues successfully even when promos are invalid

### Explicitly NOT Included

* Promo redemption enforcement
* Hard validation errors for promo issues
* Cart-level warning aggregation

## Acceptance Criteria

* [ ] Invalid promo codes do not fail the quote request and instead produce a vendor-level warning
* [ ] Vendor group warnings are persisted and returned in the `CartQuote.vendor_groups[].warnings` field

## Dependencies

* Blocked by:

  * PF-501: Validate vendor promo ownership and apply promo effects
* Blocks:

  * Phase 6: API semantics for soft warnings

## Technical Notes

* Warning type to emit:

  * `vendor_group_warning_type = invalid_promo`
* Warning JSON shape:

  ```json
  { "type": "invalid_promo", "message": "Promo code is not valid for this vendor" }
  ```
* **ASSUMPTION:** Vendor groups are persisted even when invalid, enabling warnings to attach cleanly

## Out of Scope

* Promo error localization or translation
* Client-side display logic

---

# Title [PF-164]: Wire cart quote DTOs and service into POST /cart endpoint with request validation
...
Expose the cart quote capability via a stable `POST /cart` endpoint using the new DTOs and refactored quote service.

This ticket ensures the API layer correctly:

* Accepts the minimal intent payload
* Validates request structure
* Returns an authoritative `CartQuote` response

## Scope

### Included

* Wire `QuoteCartRequest` DTO into `POST /cart` controller
* Validate request payload:

  * buyer_store_id present
  * items non-empty
* Invoke the refactored cart quote service
* Return `CartQuote` response using `newCartQuote(record)`
* Ensure endpoint is backward-incompatible with legacy upsert payloads (explicit cutover)

### Explicitly NOT Included

* Idempotency handling (separate ticket)
* HTTP error semantics for warnings (separate ticket)
* Checkout execution or conversion

## Acceptance Criteria

* [ ] `POST /cart` accepts `QuoteCartRequest` and rejects legacy upsert payloads
* [ ] Successful requests return a fully populated `CartQuote` response

## Dependencies

* Blocked by:

  * Phase 4: Cart quote service refactor
* Blocks:

  * PF-XXX: Idempotency middleware integration
  * PF-XXX: HTTP semantics normalization

## Technical Notes

* Controller location:

  * `api/controllers/cart`
* Validation library:

  * Use existing request validation pattern (`validate` tags or equivalent)
* **ASSUMPTION:** Authentication middleware already provides access to buyer context if needed

## Out of Scope

* Rate limiting
* Analytics or logging instrumentation

---

# Jira Ticket Template

## Title [PF-165]: Integrate header-based idempotency middleware for cart quote endpoint
...
Add **header-based idempotency** support to the cart quote endpoint to prevent duplicate cart creation or mutation on repeated client requests.

This ensures safe retries from clients without creating inconsistent cart state.

## Scope

### Included

* Require and process an idempotency key header (e.g. `Idempotency-Key`)
* Integrate with existing idempotency middleware or implement minimal support
* Ensure repeated requests with the same key return the same cart quote result
* Scope idempotency to buyer_store_id + endpoint

### Explicitly NOT Included

* Cross-endpoint idempotency guarantees
* Idempotency expiration tuning
* Distributed locking beyond existing middleware capabilities

## Acceptance Criteria

* [ ] Repeated `POST /cart` requests with the same idempotency key return the same cart quote
* [ ] Requests with different idempotency keys generate independent quotes

## Dependencies

* Blocked by:

  * PF-XXX: POST /cart endpoint wiring
* Blocks:
...
* Middleware may store idempotency records in Redis or DB (existing infra)
* **ASSUMPTION:** Idempotency key uniqueness is enforced per buyer_store_id
* **ASSUMPTION:** Existing idempotency middleware can be reused with minor configuration

## Out of Scope

* Idempotency for checkout or payment endpoints
* Client SDK changes

---

#  Title [PF-166]: Normalize HTTP semantics for cart quote errors vs warnings
...
Define and enforce consistent HTTP semantics for the cart quote endpoint, distinguishing **hard errors** from **soft quote warnings**.

The goal is to ensure clients can reliably interpret quote responses without losing partial results due to non-fatal issues.

## Scope

### Included

* Define hard error cases (HTTP 4xx):

  * Malformed request
  * Missing buyer_store_id
  * Invalid buyer store (KYC, type, state)
* Define soft warning cases (HTTP 200):

  * Item not available
  * Vendor invalid
  * Invalid promo
  * Quantity clamped
* Ensure warnings are returned **inside the CartQuote payload**, not as HTTP errors
* Update controller logic to reflect this distinction consistently

### Explicitly NOT Included

* Client-side interpretation of warnings
* Logging/metrics changes
* Checkout-side error handling

## Acceptance Criteria

* [ ] Structural or authorization errors return appropriate 4xx responses
* [ ] Quote-affecting issues return HTTP 200 with warnings embedded in the response payload
...
* Use existing error types (`pkgerrors.CodeValidation`, `CodeForbidden`, etc.) for hard errors
* Warnings must map to existing enum types and JSON shapes defined in Phase 1
* Only if all vendors and/or line items are invalid (after validating) then return 409.
* **ASSUMPTION:** API consumers already expect 200-with-warnings behavior from similar endpoints
...
* [ ] **Ticket:** Validate token structure (ONLY: signature + expiry)
* [ ] **Ticket:** Persist valid ad_tokens on CartRecord & Echo ad_tokens in CartQuote response & Ensure attribution does not affect pricing
...
* [ ] **Ticket:** Implement cart state transition (`active → converted`) & Generate checkout_group_id on conversion (on `CartRecord` & future `vendor_orders`)
