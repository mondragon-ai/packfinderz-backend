...
* Provide a **VendorSummary** projection for product read paths:
  ...
  * vendor `store_id`
  * vendor `company_name`
  * vendor logo (nullable): `logo_media_id` + signed/public url resolved at service layer (**ASSUMPTION**: logo is represented by a `media_attachments` row `entity_type='store'` OR a `stores.logo_media_id` if you already have it—repo should support either join path via a query helper)
...
## Title [PF-074]: Product audit logging
