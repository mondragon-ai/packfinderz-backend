...
## Title (number 0-N) => [PF-102]

Implement `stores` model with `StoreType` (buyer/vendor) and address shape
...
## Title (number 0-N) => [PF-103]

Implement `store_memberships` model and role enum
...
Implement multi-store membership join model between `users` and `stores` to support store switching and RBAC. Create `store_memberships` via Goose migration and map via GORM model. Define `member_role` as a global enum/type in `pkg/enums` (shared across auth, RBAC, controllers).

## Scope

* Goose migration to create Postgres enum `member_role` (per Data Design)
...
## Title (number 0-N) => [PF-105]

Implement refresh token storage + rotation (Redis-backed)
...
## Title (number 0-N) => [PF-106]

Implement `POST /api/v1/auth/login` endpoint (email + password)
...
* **ASSUMPTION:** “stores[]” response includes storeId + name (company/DBA) + store type
...
## Title (number 0-N) => [PF-107]

Implement `POST /api/v1/auth/register` endpoint (user + first store + owner membership)
...
* [ ] Email uniqueness enforced: registering with existing email returns 409
...
* T&C acceptance stored? **ASSUMPTION:** add `accepted_tos_at` later; for now validate boolean and ignore persistence unless already in schema
