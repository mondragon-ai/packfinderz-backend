...
## [PF-027]: Implement logout + refresh endpoints (`POST /auth/logout`, `POST /auth/refresh`)
...
* **ASSUMPTION:** logout identifies the session via Authorization bearer token. 
* Follow logging redlines
* Maybe this needs to be in the `/api/v1/prive/` routes cuz these require JWTs and the auth flow and middle ware for the `/api/v1/auth/*` routes do not expect that as its yet to be issued. 
...
## [PF-028]: Implement `POST /api/v1/auth/switch-store` endpoint (mint JWT with new `activeStoreId`)
...
* rotate refresh token & JWT.

## Acceptance Criteria

* [ ] Switching to a store the user belongs to returns 200 with a new access token containing `activeStoreId` and updated claims from the store and use for that store
...
* **ASSUMPTION:** Refresh token rotation on switch-store.
* Must not “logout old store” explicitly; switching is token refresh only. The use is still authenticated just need to validate the store authorization. 
* Please use the store, store_membership, & store tables from GORM, repos from `/internal/<domain>` & the resuable helpers documented here `docs/REUSABLE.md`.
...
## [PF-029]: Enforce store-scoped auth middleware (tenancy enforcement via `activeStoreId`)
...
## [PF-030]: Enforce RBAC permission resolution server-side
...
## [PF-031]: Add Redis-backed idempotency middleware (Idempotency-Key contract)
...
## [PF-032]: Add Redis-backed auth rate limiting (login brute force protection)
