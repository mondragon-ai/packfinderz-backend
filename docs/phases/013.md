## **Title [PF-113]: Square client bootstrap, config, and secrets management**
...
Bootstrap Square integration by adding a reusable Square client with secure configuration and secrets management. This enables subscription creation, cancellation, billing queries, and webhook verification.

## **Scope**

* Add Square client wrapper
* Load Square API keys via secrets/config
* Support test and live environments
* Expose client to API and worker layers

## **Acceptance Criteria**

* [ ] Square client initializes successfully in API and worker
* [ ] Missing or invalid secrets fail fast at startup
* [ ] Environment switching (test/live) is configurable

## **Dependencies**

* Blocked by: Square account + API keys provisioned
...
* `PACKFINDERZ_SQUARE_ACCESS_TOKEN`
* `PACKFINDERZ_SQUARE_WEBHOOK_SECRET`
* `PACKFINDERZ_SQUARE_ENV`
* `PACKFINDERZ_SQUARE_SUBSCRIPTION_PLAN_ID`
* **ASSUMPTION:** secrets are injected via env vars (no Vault in MVP)

## **Out of Scope**

* Multi-account Square support
...
Create or verify all database tables required for Square-based subscriptions and billing history. These tables persist Square state locally and act as the source of truth for gating and reporting.
...
* Migrations for:
  ...
  * `subscriptions`
  * `payment_methods` // Probaby should hash the PM from Square while in storage -- similar to the password, right? 
...
* Key fields (subscriptions):
  ...
  * `status`
  * `current_period_end`
* **ASSUMPTION:** one active subscription per vendor store (1 → 0/1)
...
## **Title [PF-116]: Square webhook consumer for subscription state sync**
...
Add a Square webhook endpoint to consume subscription lifecycle events and mirror Square truth into local tables. This endpoint is authoritative for subscription state changes.

## **Scope**

* Webhook endpoint (e.g. `/api/v1/webhooks/square`)
...
* [ ] Subscription state updates correctly on Square events
* [ ] `stores.subscription_active` mirrors Square truth
...
* Idempotency key: Square `event.id`
...
* Non-subscription Square events

---

## **Title [PF-117]: Vendor billing history endpoint (charges)**
...
* [ ] Results are consistent with Square/local records
...
* Data source: local `charges` table (mirrored from Square)
...
## **Title [PF-118]: Enforce subscription gating across marketplace**
