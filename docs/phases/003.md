...
* **ASSUMPTION:** Hard delete allowed for now if not referenced in licenses, COAs, or manifests.
...
* [ ] URL expires based on config (e.g. 5–15 min)
...
* **ASSUMPTION:** TTL configured via `PACKFINDERZ_GCS_READ_URL_EXPIRY`
...
* Define canonical `owner_type` values
