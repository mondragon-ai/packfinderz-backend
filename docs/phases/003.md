...
This endpoint now relies on the `public_url` stored on each `media` row (exposed as `signed_url`). The GCS consumer builds the permanent `storage.googleapis.com` link via `pkg/storage/gcs.PublicURL` when the object is finalized, so clients can cache the URL indefinitely instead of refreshing signed URLs every 30 minutes.
