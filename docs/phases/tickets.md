...
- **Phase 11 — Billing + Subscriptions + Square (Production-Ready)**
  **Goal:** Replace Square stubs with real API integrations, add plan + card + subscription primitives, reconcile via webhooks, and implement nightly ad usage billing into `usage_charges`/`subscriptions` with proper guards and logging.
  ...
  - [x] Ticket [PF-261]: Add `billing_plans` table + GORM model + repo/service (local source-of-truth for plans with Square mapping IDs, interval/price/trial, status, is*default, and UI metadata JSON) + indexes (`status`, `is_default`, `square*\*`)
...
- [x] Ticket [PF-275]: Goose migration — create ONLY `wishlists` table (`id`, `store_id`, `created_at`) with `unique(store_id)` index and FK to `stores(id) ON DELETE CASCADE` & GORM models inside `pkg/db/models/wishlist.go` matching the migration shapes with proper struct tags and associations
- [x] Ticket [PF-276]: Repo — `internal/wishlist/repo.go` implementing `GetOrCreateWishlist(storeID)`, `AddItem(wishlistID, productID)`, `RemoveItem(wishlistID, productID)`, `ListItems(wishlistID, cursor/limit)` returning `WishlistItem` rows with product data preloaded (from products tables to mirror the BrowseProducts endpoints), and `ListItemIDs(wishlistID)` returning only product IDs for fast heart-icon rendering & DTOs — `internal/wishlist/types.go` defining `WishlistDTO`, `WishlistItemDTO` (with `ProductSummary` projection), and `WishlistIDsDTO`
...
- **Phase 17 — Store**
  - [x] [PF-293]: Implement GET /v1/stores/{store_id} Controller (Authenticated Viewer Access)
...
- **Phase 1 — Media Processing Worker (Compression + OCR)**
  **Goal:** Automated derived assets and OCR pipeline (beyond core upload/delete).
  - [ ] Ticket [PF-XXX]: Create `cmd/media-worker` binary
  - [ ] Ticket [PF-XXX]: Subscribe media-worker to GCS finalize events (env-driven)
  - [ ] Ticket [PF-XXX]: Implement image/video compression helper
  - [ ] Ticket [PF-XXX]: Implement OCR provider abstraction (OpenAI vs Document AI)
  - [ ] Ticket [PF-XXX]: Implement OCR text generation + store derived asset (`ocr.txt`)
  - [ ] Ticket [PF-XXX]: Update media row with derived artifacts
  - [ ] Ticket [PF-XXX]: Emit `media_processed` outbox event

- **Phase 2 — Notifications: Email Pipeline**
  **Goal:** Send email notifications via adapter interface (SendGrid later).
  - [ ] Ticket [PF-XXX]: Set up sendgrid client and wire up to notifications pubsub consumer
  - [ ] Ticket [PF-XXX]: Define notification email templates
  - [ ] Ticket [PF-XXX]: Define email sender interface
...
- [ ] Ticket [PF-XXX]: Implement OpenAI client bootstrap (shared with extraction if not already)
- [ ] Ticket [PF-XXX]: Implement COA OCR → structured parser
- [ ] Ticket [PF-XXX]: Define ProductDraft DTO (name, potency, batch, lab, etc.)
- [ ] Ticket: Map COAExtraction → ProductDraft prompt input
- [ ] Ticket [PF-XXX]: Implement product draft JSON generator from parsed COA
- [ ] Ticket: Validate ProductDraft schema
- [ ] Ticket [PF-XXX]: Persist product draft + status (draft / ready / rejected)
...
- **Phase 4 — Checkout Refactor Safety Locks (Docs/Flags)**
  **Goal:** Formalize sequencing and guardrails for future checkout iterations.
  - [ ] Ticket [PF-XXX]: Write `CHECKOUT_REFACTOR.md` sequencing locks
  - [ ] Ticket [PF-XXX]: Add feature flag/config guard for selecting checkout flow entrypoint (if parallel flows exist)
  - [ ] Ticket [PF-XXX]: Commit grep checklist for all references to `checkout_groups` and `AttributedAdClickID`

- **Phase 5 — Optional Admin/Vendor Finance UX**
  **Goal:** Convenience endpoints that don’t affect authoritative finance state.
  - [ ] Ticket [PF-XXX]: Implement optional vendor “confirm paid” endpoint (audited; non-authoritative)
...
- **Phase 1 — Ads Engine (Full CPM System)**
  **Goal:** Full CPM ad engine with serve/track, tokens, checkout attribution, rollups, billing fanout, analytics.
  - [ ] Ticket [PF-XXX]: Define ad engine constants + Redis key schema + TTL conventions
  - [ ] Ticket [PF-XXX]: Add/confirm ad enums (status, placement, billing model, token event type, token target type)
  - [ ] Ticket [PF-XXX]: Add migration for `cart_records.attribution_tokens` JSONB + indexes
  - [ ] Ticket [PF-XXX]: Add migration for `vendor_orders.attribution` JSONB + optional indexable fields
  - [ ] Ticket [PF-XXX]: Add migration for `vendor_order_line_items.attribution` JSONB + optional indexable fields
  - [ ] Ticket [PF-XXX]: Add `ad_daily_rollups` table schema (unique ad_id+day + indexes)
  - [ ] Ticket [PF-XXX]: Ensure `usage_charges` uniqueness supports idempotent daily ad spend (store_id+type+for_date)
  - [ ] Ticket [PF-XXX]: Define attribution token schema (versioning, size constraints, required fields)
  - [ ] Ticket [PF-XXX]: Implement token signing + verification utility (HMAC/JWT HS256) with strict validation rules
  - [ ] Ticket [PF-XXX]: Implement server-side token validation helper (signature/expiry/buyer_store match/enums/dedupe)
  - [ ] Ticket [PF-XXX]: Define deterministic precedence rules (click>impression, recency, stable tie-break)
  - [ ] Ticket [PF-XXX]: Implement repo to fetch eligible ads for `/ads/serve` (status/placement/time window + gating joins)
  - [ ] Ticket [PF-XXX]: Implement service eligibility filters (subscription_active, kyc verified, status/time window, geo hook)
  - [ ] Ticket [PF-XXX]: Implement Redis budget gate evaluation vs daily_budget_cents
  - [ ] Ticket [PF-XXX]: Implement serve algorithm (highest bid wins + deterministic tie-break)
  - [ ] Ticket [PF-XXX]: Implement serve DTOs (creative + signed impression/click tokens + request_id)
  - [ ] Ticket [PF-XXX]: Implement route `GET /ads/serve`
  - [ ] Ticket [PF-XXX]: Implement Redis impression dedupe helper (SETNX with TTL)
  - [ ] Ticket [PF-XXX]: Implement route `POST /ads/impression` (verify token + increment Redis imps + spend with dedupe)
  - [ ] Ticket [PF-XXX]: Implement Redis click dedupe helper (SETNX with TTL)
  - [ ] Ticket [PF-XXX]: Implement route `GET /ads/click` (verify token + increment clicks + 302 redirect)
  - [ ] Ticket [PF-XXX]: Update cart DTOs to accept bounded `attribution_tokens[]`
  - [ ] Ticket [PF-XXX]: Normalize tokens on cart save (validate/dedupe/cap; keep most recent per key)
  - [ ] Ticket [PF-XXX]: Persist normalized `cart_records.attribution_tokens` and return normalized set to client
  - [ ] Ticket [PF-XXX]: Add guardrails for payload size + logs for invalid tokens; drop invalid tokens without failing checkout
  - [ ] Ticket [PF-XXX]: Compute per-vendor-order attribution candidates from cart tokens at checkout-time
  - [ ] Ticket [PF-XXX]: Materialize order-level attribution into vendor_orders.attribution (vendor_store_id match)
  - [ ] Ticket [PF-XXX]: Materialize line-item attribution into line_items.attribution (product_id match)
  - [ ] Ticket [PF-XXX]: Persist deterministic attribution reasons for debugging
  - [ ] Ticket [PF-XXX]: Persist attribution within same checkout transaction (no partial writes)
  - [ ] Ticket [PF-XXX]: Implement daily rollup job (read Redis day N → write ad_daily_rollups + usage_charges idempotently)
  - [ ] Ticket [PF-XXX]: Implement deterministic rounding policy helper for spend calculations
  - [ ] Ticket [PF-XXX]: Emit outbox event `ad_spend_rolled_up` after successful rollup transaction
  - [ ] Ticket [PF-XXX]: Add Pub/Sub topic wiring and consumer skeletons for analytics + billing
  - [ ] Ticket [PF-XXX]: Implement billing consumer to bridge daily usage_charges into Stripe usage/charges (stub allowed)
  - [ ] Ticket [PF-XXX]: Define analytics payload contract for ads (rollups + checkout attribution snapshot)
  - [ ] Ticket [PF-XXX]: Implement analytics consumer inserts for order attribution into BigQuery ad tables
  - [ ] Ticket [PF-XXX]: Implement analytics consumer inserts for rollups into BigQuery rollup tables
  - [ ] Ticket [PF-XXX]: Ensure one row per attributed ad per line item plus optional order-level store row
  - [ ] Ticket [PF-XXX]: Define failure mode behavior for Redis outages (serve none; fail closed) with structured logs
  - [ ] Ticket [PF-XXX]: Add observability for serve/track (candidate counts, exclusion reasons, dedupe hits, winner, budget gating)
  - [ ] Ticket [PF-XXX]: Add load shedding (candidate limit + optional short-lived caching)
  - [ ] Ticket [PF-XXX]: Add integration tests for serve→impression→click→cart token persist→checkout attribution→rollup→usage_charges
...
- **Phase 3 — Ops, Observability, Hardening**
  **Goal:** Operational safety improvements beyond MVP.
  - [ ] Ticket [PF-XXX]: Implement worker metrics and DLQ visibility views/queries
  - [ ] Ticket [PF-XXX]: Write replay and recovery runbooks (DLQ replay, idempotency expectations)
  - [ ] Ticket [PF-XXX]: Add/standardize feature flags for risky rollouts
  - [ ] Ticket [PF-XXX]: Perform backup/restore drills and document procedure
