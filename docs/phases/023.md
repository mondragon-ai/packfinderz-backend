...
* Even license doc, if pending past 7 days its able to be deleted
...
# [INCPMPLETE ❌] Title [PF-205]: Detach attachment references and delete GCS artifacts on media deletion
...
# Title [PF-206]: Bug - Fix media delete endpoint not deleting DB or GCS objects
...
Fix an issue where the media delete endpoint returns HTTP success but does not actually delete the media record or the underlying GCS object. Ensure end-to-end correctness and observability.

## Scope

* Verify delete flow from API → worker → GCS  →  (worker binary subscirbed to GCS bucket reads deleted object)
* Ensure DB row deletion occurs -> DO NOT UPDATE AS DELETED STATE, ACTUALLY DELETE
* Ensure GCS object deletion occurs **IMPORTANT**
