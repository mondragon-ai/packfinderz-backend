...
* [x] `DELETE /api/v1/stores/me/users/{userId}` removes membership
* [x] Removing last owner is rejected
* [x] User account itself is not deleted
...
* **Implementation:** new `/api/v1/stores/me/users/{userId}` handler + service guard that counts `owner` memberships before deleting the row
