...
Implement an authenticated endpoint to fetch the **active store’s full profile** based on `activeStoreId` from JWT. This endpoint returns **non-public internal fields** and is used for store settings and internal dashboards.

## Scope

* Fetch store by `activeStoreId`
* Return full store profile (excluding address mutation fields)
* Enforce store-scoped authorization

## Acceptance Criteria

* [ ] `GET /api/v1/stores/me` returns store data for `activeStoreId`
* [ ] Request without valid JWT or `activeStoreId` is rejected
...
* Update allowed store fields (e.g., `description`, `phone`, `email`, `social`)
* Enforce role-based access (owner/admin only)
...
* [ ] Only authorized roles can access this endpoint
* [ ] Each entry includes user id, email, name, role, created_at
