...
Here’s the **single consolidated ticket** for **Phase 3 — Presigned Upload Flow (Client → GCS)**, written to **match your example exactly**, and explicitly **building on the fact that the GCS client already exists** and `media_kind` enums are already defined.

---
Perfect — based on your **final answers + existing commits (PF-038, PF-039)**, here is a **clean, dependency-ordered backend-only ticket list**, following **your exact lifecycle**, **no finalize endpoint**, **Pub/Sub–driven verification**, and **combined where it makes sense**.

I am **not** inventing frontend work, and I’m explicitly aligning with your state diagram + sequence diagram.

---

# 📦 Media + Upload System — Remaining Backend Tickets

---

## 🟢 Already Completed (for context)

* ✅ **PF-038** — Canonical metadata persistence (licenses groundwork)
* ✅ **PF-039** — Bootstrap GCS client for API + worker
...
## [PF-044]: Implement Pub/Sub consumer for GCS OBJECT_FINALIZE to mark Media rows as uploaded
...
## [PF-047] Signed READ URL resolution for media (API helper + endpoint)
...
## [PF-048] Media deletion flow (API + worker)
...
## [PF-049]: Implement license metadata creation endpoint
...
## [PF-050] Implement GCS presigned upload helper + endpoint (licenses)
...
## [PF-051] Implement license finalize endpoint
...
## [PF-052] Implement list licenses endpoint (active store)
...
## [PF-053] Implement delete license endpoint
...
## [PF-054] Implement admin license verify/reject endpoint
...
## [PF-055] Mirror license status changes to store KYC status
...
## [PF-055] Add license expiry scheduler + compliance notifications
