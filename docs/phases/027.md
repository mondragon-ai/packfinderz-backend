...
# Title [PF-245]: Set payment intent settled state and emit cash_collected outbox event
...
# Title [PF-246]: Reject cash collection when order is not dispatch-ready & create endpoint (`POST /agent/orders/{orderId}/cash-collected`)
...
# Title [PF-247]: Prevent duplicate cash collection on already settled or paid orders
...
# Title [PF-248]: Mark payment intent as failed when cash collection validation fails
...
# [INCOMPLETE ❌] Title [PF-249]: Support rejected payment state for future ACH or admin-declined settlements
...
# Title [PF-250]: Emit payment_failed or payment_rejected outbox events for downstream consumers
