...
# [INCOMPLETE ❌] Title [PF-244]: Implement agent cash-collected endpoint and append ledger cash_collected event
...
* [ ] Agent can mark an order as cash collected
* [ ] `ledger_events` record with type `cash_collected` is appended
* [ ] Endpoint is idempotent (duplicate calls do not create multiple ledger rows)
...
# [INCOMPLETE ❌] Title [PF-2XX]: Set payment intent settled state and emit cash_collected outbox event
