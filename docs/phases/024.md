...
# Title [PF-212]: Add low stock threshold and max quantity constraints
...
* [ ] Goose migration & DB model updates + helpers/ mapping and global usecases implimented (mostly clamping price on cart quote for max quantity)
...
# Title [PF-213]: Convert volume discount to percentage-based pricing
...
Extend the product media model to include a direct `media_id` reference, enabling stronger linkage with the media system and attachment lifecycle. THis is also used for detatching references if media is deleted.

## Scope

* DB migration to add `media_id` to Product Media model
...
Current ProductMedia model for products
```go
package models
