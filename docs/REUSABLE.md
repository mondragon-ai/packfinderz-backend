...
* `POST /api/v1/agent/orders/{orderId}/cash-collected` lets the assigned agent record that cash was collected. `internal/orders.Service.AgentCashCollected` now marks the `payment_intents.status=settled`, stamps `cash_collected_at`, zeros `balance_due_cents`, records the assignment’s `cash_pickup_time`, emits a single `cash_collected` outbox event for downstream consumers, and appends one `ledger_events(type=cash_collected)` row so duplicate requests become no-ops and the HTTP surface stays idempotent.
