...
* `POST /api/v1/agent/orders/{orderId}/cash-collected` lets the assigned agent record that cash was collected; the handler calls `internal/orders.Service.AgentCashCollected`, which guarantees a single `ledger_events(type=cash_collected)` row per order so duplicate requests become no-ops while the `Idempotency-Key` middleware keeps the HTTP surface idempotent as well.
