...
* Attribution tokens referenced in `ad_tokens` must be valid JWTs (signature + expiry) before they land on the cart record or the `CartQuote` response; invalid tokens are filtered out without affecting the quote.
