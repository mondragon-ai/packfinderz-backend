...
* Attribution tokens referenced in `ad_tokens` must be valid JWTs (signature + expiry) before they land on the cart record or the `CartQuote` response; they are normalized via `token.NormalizeTokens` (signature/expiry/buyer_store_id, deduplicated by `(event_type,target_type,target_id,ad_id)`, capped at 50 entries per cart) so invalid or duplicate tokens are dropped silently while the stored bag stays bounded.
...
* `vendor_orders.ad_token` captures the selected store-level attribution token, and `order_line_items.ad_token` stores the per-product attribution array. `internal/checkout/service` revalidates the normalized `cart_records.ad_tokens`, deterministically picks `click` > `impression` (then recency/tie-break) per vendor/product target, and writes the chosen tokens into these columns so analytics can reconstruct which ads drove each order/line item.
...
* `internal/checkout.Repository` (powered by `orders.Repository` + `cart_records`) exposes `FindByCheckoutGroupID`/`FindByCartID`, returning the aggregated checkout group (vendor orders, payment intents, assignments, cart vendor groups) so post-checkout endpoints can poll the latest status by either identifier.
* Checkout enforces the cart quote’s `valid_until`: the cart must include a non-zero expiry and any stale quote is rejected with a conflict before the conversion logic runs so the UI must refresh the quote.
* Cart conversions are idempotent: once a cart status flips to `converted`, subsequent checkout attempts simply replay the previously created `CheckoutGroup`/vendor orders instead of mutating the cart again.
...
* `internal/products.ProductSummary` and `ProductListResult` represent the browse payload plus the richer `pagination` metadata (`page`, `total`, `current`, `first`, `last`, `prev`, `next`).
...
* `CreateLicense`/`DeleteLicense` both call `internal/media.AttachmentReconciler` so each license writes a `media_attachments` row with `entity_type='license'`, protecting the referenced `media_kind=license_doc` asset until the attachment is removed inside the same transaction as the license mutation.
...
* Ad query helpers reuse `types.AdAnalyticsMetrics`, `types.AdAnalyticsSeriesPoint`, and `types.AdQueryResponse` so the ads detail endpoint can mirror the analytics warehouse payload without reinventing the DTO surface.
...
* `internal/orders.Service.LineItemDecision` loads the order + line item, checks vendor ownership, updates the line item status, releases inventory for rejects, recomputes `subtotal_cents`, `total_cents`, and `balance_due_cents`, sets `fulfillment_status`/`status` to `partial`/`fulfilled` and `ready_for_dispatch` when all `pending` line items are resolved, and emits the new `order_ready_for_dispatch` outbox event (`enums.EventOrderReadyForDispatch`; pkg/enums/outbox.go:57-72).
* `POST /api/v1/agent/orders/{orderId}/cash-collected` lets the assigned agent record that cash was collected. `internal/orders.Service.AgentCashCollected` now marks the `payment_intents.status=settled`, stamps `cash_collected_at`, zeros `balance_due_cents`, records the assignment’s `cash_pickup_time`, emits a single `cash_collected` outbox event for downstream consumers, appends one `ledger_events(type=cash_collected)` row, and persists a failure reason whenever validation fails so duplicate requests are rejected once the intent hits any terminal status (`settled|paid|failed|rejected`) and operators can review failed intents before retrying; unrecoverable failures now emit `payment_failed` events (future `payment_rejected` transitions will emit `payment_rejected`).
...
* `GET /api/v1/vendor/products` is the vendor-only table view: it reuses `ProductListFilters`/`ProductListResult`, applies the same filters + pagination, and restricts the query to `store_id = activeStoreId` without enforcing buyer visibility so internal management can see all store-owned products (internal/products/repository.go:229-330; api/controllers/products.go:223-327).
* `internal/products` now captures `max_qty` and `inventory.low_stock_threshold` on the models and validates them at the service layer so catalog tooling can honor per-line caps and low-stock guardrails without re-querying derived state.
...
* `internal/cron/pending_media_cleanup_job.go` implements PF-204: it queries `media.status='pending'` rows older than seven days via `media.Repository.ListPendingBefore`, deletes all corresponding `media_attachments`, removes the media rows inside the transaction, and logs `media_deleted`/`attachments_deleted` so abandoned uploads no longer hang around.
