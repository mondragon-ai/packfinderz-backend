...
* `internal/cron/notification_cleanup_job.go` implements PF-139: it computes `now - 30 days`, calls `notifications.Repository.DeleteOlderThan(ctx, tx, cutoff)` inside `db.WithTx`, and logs `rows_deleted` so the `notification-cleanup` job keeps the `notifications` table bounded without affecting other cron jobs.
