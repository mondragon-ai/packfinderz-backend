 ...
 * Dead-letter persistence: `pkg/outbox/DLQRepository` writes terminal events into the append-only `outbox_dlq` table (stores the original envelope plus `error_reason`, `error_message`, and the `attempt_count` that triggered the terminal mark) so manual remediation tooling can replay or diagnose failures without touching the pending queue.
