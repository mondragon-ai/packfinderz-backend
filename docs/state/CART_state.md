...
- JWT configuration (`cmd/api/main.go:67-183`, `api/middleware/auth.go:16`) still powers authentication, but the ads configuration (`cmd/api/main.go:67-183`, `cmd/api/main.go:235`) now passes `cfg.Ads.TokenSecret` into `pkg/ads/token.NewParser`, and both `internal/cart` and `internal/checkout` reuse that parser so attribution tokens are validated (signature, expiry, buyer_store_id) before they land on the cart record or are attributed to vendor orders/line items.
...
- `token.NormalizeTokens` validates signatures/expiry/buyer_store_id, deduplicates entries by `(event_type,target_type,target_id,ad_id)`, and caps the bag at 50 tokens before the normalized set lands on `cart_records.ad_tokens`, so the stored snapshot is bounded and only the most recent tokens per target survive (`pkg/ads/token/normalize.go`, `internal/cart/service.go`).
