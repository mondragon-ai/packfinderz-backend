...
- **Square:** The API and worker both instantiate `pkg/square.NewClient` (`cmd/api/main.go:54`, `cmd/worker/main.go:48`) to ensure the access token/webhook secret/environment are present (`pkg/square/client.go:31`), and subscriptions now call `subscriptions.NewSquareClient` so the shared wrapper hits real Square APIs instead of the old stub client. Subscription creation/cancellation also needs `PACKFINDERZ_SQUARE_LOCATION_ID` so Square knows which location to bill.
...
- **Square API integration now hits production Square.** All subscription create/cancel/get workflows call `subscriptions.NewSquareClient` (`internal/subscriptions/square_client.go:1`), which uses `pkg/square` to issue the real HTTP requests instead of the previous stub.
