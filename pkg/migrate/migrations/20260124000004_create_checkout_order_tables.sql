...
CREATE TABLE IF NOT EXISTS checkout_groups (
  ...
  CONSTRAINT checkout_groups_ad_click_fk FOREIGN KEY (attributed_ad_click_id) REFERENCES ad_clicks(id) ON DELETE SET NULL
);
...
CREATE TABLE IF NOT EXISTS vendor_orders (
  ...
  CONSTRAINT vendor_orders_checkout_fk FOREIGN KEY (checkout_group_id) REFERENCES checkout_groups(id) ON DELETE CASCADE,
  CONSTRAINT vendor_orders_buyer_fk FOREIGN KEY (buyer_store_id) REFERENCES stores(id) ON DELETE RESTRICT,
  CONSTRAINT vendor_orders_vendor_fk FOREIGN KEY (vendor_store_id) REFERENCES stores(id) ON DELETE RESTRICT,
  CONSTRAINT vendor_orders_buyer_vendor_diff CHECK (buyer_store_id <> vendor_store_id)
);
...
CREATE TABLE IF NOT EXISTS order_line_items (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT order_line_items_order_fk FOREIGN KEY (order_id) REFERENCES vendor_orders(id) ON DELETE CASCADE,
  CONSTRAINT order_line_items_product_fk FOREIGN KEY (product_id) REFERENCES products(id) ON DELETE SET NULL
);
...
CREATE TABLE IF NOT EXISTS payment_intents (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT payment_intents_order_fk FOREIGN KEY (order_id) REFERENCES vendor_orders(id) ON DELETE CASCADE
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_payment_intents_order_id ON payment_intents (order_id);
