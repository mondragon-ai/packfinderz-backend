...
* `GET /api/v1/media`
  ...
  * Success: `200`
  * Errors: `401, 403`
* `DELETE /api/v1/media/{mediaId}` removes media rows whose status is `uploaded` or `ready`, deletes the GCS object (ignoring already-missing objects), and marks the row as `deleted`; rejects other stores or statuses with `403`/`409`.
