...
* `billing_plans` introduces local plan metadata (status, subscription interval, Square plan mapping, trial rules, UI badge/description/bullets, optional feature gates, and default/test flags) and exposes the canonical plan list/lookup helpers plus `FindDefaultBillingPlan`, keeping the default plan deterministic without re-querying Square (`pkg/migrate/migrations/20270104000000_create_billing_plans.sql:1-49`; `pkg/db/models/billing_plan.go:7-27`; `internal/billing/repo.go:26-122`; `internal/billing/service.go:65-125`).
* Admin-only `/api/admin/v1/billing/plans` CRUD and vendor-only `/api/v1/vendor/billing/plans[/{planId}]` read routes expose the canonical plan catalog to clients without round-tripping to Square; `api/controllers/billing/plans.go` handles validation/response shaping and delegates to `internal/billing.Service`.
...
* `cmd/outbox-publisher/service.go` `Run` ensures the DB and Pub/Sub pings succeed, then loops: `processBatch` runs inside `db.WithTx`, calls `outbox.Repository.FetchUnpublishedForPublish` (claims `published_at IS NULL` rows with `FOR UPDATE SKIP LOCKED`, respects `Config.Outbox.BatchSize`/`MaxAttempts`, and orders by `created_at ASC, id ASC`), publishes each event sequentially, logs structured context per attempt (`outbox_id`, `event_type`, `aggregate_type`, `aggregate_id`, `batch_size`, `attempt_count`, optional `event_id`, `occurred_at`, `last_error`), then calls `MarkPublishedTx` or `MarkFailedTx` (incrementing `attempt_count`/truncating `last_error`) so a single failure never halts the dispatcher; idle/error loops sleep with `sleep`/`nextBackoff` + `withJitter` (base `Config.Outbox.PollIntervalMS`, default 500ms, cap 10s, plus 0-250ms jitter) instead of spinning (cmd/outbox-publisher/service.go:66-235; pkg/outbox/repository.go:20-101).
  * PF-142 will extend the dispatcher to consult `pkg/outbox/registry` before publishing: the registry maps each `event_type` to one canonical topic, the expected aggregate, and the typed payload struct (located in `pkg/outbox/payloads`), decodes/validates the `payload_json`, enforces `aggregate_type`/`aggregate_id` presence, and rejects unknown or invalid `event_type`s as terminal failures so the future DLQ plumbing can capture them, all while only reading the stored outbox row with no extra lookups.
  * PF-144 introduces the `outbox_dlq` persistence model: a goose migration creates the append-only table, `pkg/outbox/dlq_repository.go` writes rows containing the exact stored envelope (`event_id`, `event_type`, `aggregate_type`, `aggregate_id`, `payload_json`) plus `error_reason`, `error_message`, and `failed_at`, and the dispatcher will persist that row atomically alongside the terminal mark so remediation tooling always sees the raw envelope without replaying the regular queue.
  * PF-145 wires that persistence into the dispatcher’s control flow: when a non-retryable error occurs or `attempt_count >= Config.Outbox.MaxAttempts`, the worker builds the same Pub/Sub envelope, writes the DLQ row before marking the outbox event terminal, and `FetchUnpublishedForPublish` skips any terminal `Flags` so the row never re-enters the publish loop.
...
> `internal/analytics/router` splits the envelope per canonical `event_type`, decodes the typed payload, and dispatches to handler stubs so the BigQuery writer interface remains consistent across the analytics surface.
...
* `cmd/outbox-publisher/main` boots config/logging/DB/PubSub, sets `cfg.Service.Kind=outbox-publisher`, builds `outbox.Repository`, and runs until interrupted so the dispatcher keeps the async pipeline flowing (cmd/outbox-publisher/main.go:1-72; cmd/outbox-publisher/service.go:66-235).
* `cmd/cron-worker/main` loads config/logging, Postgres, and Redis, registers the cron job registry, and runs a 24h ticker while holding a global Redis lock so only the leader executes the jobs; job start/end/duration logs plus Prometheus metrics (`job_duration_seconds`, `job_success`, `job_failure`) monitor scheduler health without touching Pub/Sub or event streams (`internal/cron/*.go`).
...
* `POST /api/admin/v1/auth/register` – dev-only helper (mounted only when `PACKFINDERZ_APP_ENV != prod`) that creates a `system_role="admin"` user, hashes the supplied password, and flows through the admin login helper so the response mirrors the login tokens + `X-PF-Token` header while returning `409 Conflict` on duplicate emails.
* `POST /api/v1/auth/register` – requires `Idempotency-Key`, creates user + store, auto-logins, and returns tokens + `X-PF-Token`. When the email already exists the password must match the existing account and the endpoint instead creates an additional store + owner membership for that user (api/controllers/register.go:13-41; internal/auth/register.go:21-133).
...
* Signed read URLs for `uploaded`/`ready` media are generated by the media service helper and expire per `PACKFINDERZ_GCS_DOWNLOAD_URL_EXPIRY`.
* Signed read URLs for `uploaded`/`ready` media are generated by the media service helper and expire per `PACKFINDERZ_GCS_DOWNLOAD_URL_EXPIRY`.
...
* `DELETE /api/v1/media/{mediaId}`
  ...
  * Loads every `media_attachments` row, rejects the request immediately if any attachment’s `entity_type` is in `ProtectedAttachmentEntities` (`license`/`ad`), emits the downstream delete event only after this guard passes, and allows the hard delete only when no protected attachments reference the media (non-protected attachments remain for the worker to reconcile later).
  * Success: `204`
  * Errors: `401, 403, 404, 409`
...
* The `POST /api/v1/cart` endpoint now relies on `internal/cart.Service.QuoteCart` to gate the request: the buyer store must be type `buyer` and `kyc_status=verified`, each vendor store must be verified, subscribed, and share the buyer’s state, every product SKU/unit ties back to the active vendor listing, and MOQ/inventory limits plus volume tiers are rechecked before the cart record and its items are persisted. Idempotent requests reuse the same Redis key for 24h while the stored authoritative cart quote (including per-vendor `cart_vendor_groups`, item warnings, and any `ad_tokens` attribution) is returned as the canonical summary. Attribution tokens are validated as JWTs (signature + expiry) and scoped to the submitting buyer store before they are normalized (deduplicated by `(event_type,target_type,target_id,ad_id)` and capped at 50 entries via the `PACKFINDERZ_ADS_TOKEN_SECRET` parser) so invalid, expired, or unexpected tokens are silently dropped from the cart record and response without impacting pricing. Checkout now enforces the `valid_until` timestamp: the cart record must include a non-zero expiry and any expired quote is rejected before checkout runs, forcing the client to re-quote, while carts transition to `converted` exactly once so replaying a converted cart returns the previously created checkout group instead of mutating it again.
...
* Requires the active store to be a buyer (buyer stores only can checkout) and expects `cart_id`, `shipping_address`, and `payment_method` in the request body (an optional `shipping_line` may also be provided when the cart already selected a shipping option). The legacy `attributed_ad_click_id` field is no longer accepted; checkout attribution now flows from `cart_records.ad_tokens`, which are revalidated, grouped by vendor-store and product-target scopes, and deterministically selected (click > impression, recency, stable tie-break) so the chosen tokens are persisted onto `vendor_orders.ad_token` and `order_line_items.ad_token` inside the checkout transaction for downstream analytics.
...
* Buyers can now call `GET /api/v1/checkout/{identifier}/confirmation`, where `identifier` is either the `checkout_group_id` saved during checkout or the originating `cart_id`. `internal/checkout.Repository` composes `orders.Repository` and the `cart_records` snapshot to replay the checkout group (vendor orders, payment intents, assignments, cart vendor groups) atomically; the controller (`api/controllers/checkout_confirmation.go`) simply returns that aggregate without mutating state so the buyer can refresh what each vendor is doing after checkout.
...
* Vendors can also call `GET /api/v1/vendor/billing/plans` (and `/api/v1/vendor/billing/plans/{planId}`) to fetch the curated, active `billing_plans` catalog (price, trial metadata, features, UI badge) without hitting Square.
* Admins manage the canonical plans via the new `/api/admin/v1/billing/plans` group (GET list, POST create, PATCH update, DELETE hide), so plan metadata + the Square selling-plan ID can be edited while the service enforces RBAC and the `internal/billing.Service` conversions.
...
* `GET /api/v1/vendor/payment-methods`
  ...
  * Resolves the vendor store via `vendorcontext.ResolveVendorStoreID` (ensuring the caller holds one of the billing roles) and reads every row via `billing.Repository.ListPaymentMethodsByStore(storeID)` where the repo already orders by `created_at DESC`.
...
* The reviews service (`internal/reviews/service`) validates that the buyer-user holds an active membership with the buyer store and that a delivered/closed vendor order already links the buyer to the vendor; only then does it persist the review and mark `is_verified_purchase=true`.
* `ListVisibleReviews` exposes the paginated, cursor-based feed for a vendor store and filters to `is_visible=true` rows while reusing the standard `Page/Total/Current/First/Last/Prev/Next` metadata so the storefront can consistently render review pages.
